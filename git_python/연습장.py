...
# # b2 = Button(window, text="섭씨->화씨", command=pro)
...
# # sev = Button(window,text="8", command=lambda: e1.insert("end",str(8)))
# # sev.grid(row=4, column=0)
# #/Users/kangbyeongjin/Downloads/포켓몬스터 소울실버(K).nds
# import  ndspy.lz10
# compressed = ndspy.lz10.compress(b"/Users/kangbyeongjin/Downloads/aaa.nds")
# compressed
...
def main():
    root = tk.Tk()
    root.title("Canvas 분리 예제")
    root.geometry("600x400")

    # 상단 Canvas
    top_canvas = tk.Canvas(root, bg="lightblue", height=200)
    top_canvas.pack(side=tk.TOP, fill=tk.BOTH, expand=True)

    # 하단 Canvas
    bottom_canvas = tk.Canvas(root, bg="lightgreen", height=200)
    bottom_canvas.pack(side=tk.BOTTOM, fill=tk.BOTH, expand=True)

    # 상단 Canvas에 그리기
    top_canvas.create_rectangle(50, 50, 150, 150, fill="blue")
    top_canvas.create_text(100, 30, text="Top Canvas", font=("Arial", 16), fill="black")

    # 하단 Canvas에 그리기
    bottom_canvas.create_oval(50, 50, 150, 150, fill="red")
    bottom_canvas.create_text(100, 30, text="Bottom Canvas", font=("Arial", 16), fill="black")
